...
d("E2E — customer journey (register → review)", () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.user.update({
      where: { email: "admin@dnr.test" },
      data: { role: UserRole.ADMIN },
    });

    const techReg = await request(app.getHttpServer())
      .post(api("/auth/register"))
      .send({
        email: "tech@dnr.test",
        password: "P@ssw0rd123",
    ...
    const techUser = await prisma.user.update({
      where: { email: "tech@dnr.test" },
      data: { role: UserRole.TECHNICIAN },
    });
    const techProfile = await prisma.technicianProfile.create({
      data: { userId: techUser.id, isAvailable: true },
    });
  ...
  it("2. logs the customer in", async () => {
    const res = await request(app.getHttpServer())
      .post(api("/auth/login"))
      .send({ email: "journey@dnr.test", password: "P@ssw0rd123" });
    expect(res.status).toBe(201);
  ...
  it("3. creates an address then a booking", async () => {
    ...
    const booking = await request(app.getHttpServer())
      ...
      .send({
        serviceId,
        addressId,
        scheduledWindowStart: new Date(Date.now() + 86400_000).toISOString(),
        scheduledWindowEnd: new Date(Date.now() + 90000_000).toISOString(),
      });
  ...
  it("6. creates a payment intent for the booking", async () => {
    ...
    expect([200, 201]).toContain(res.status);
    expect(
      res.body.client_secret ?? res.body.data?.client_secret,
    ).toBeDefined();
  });
