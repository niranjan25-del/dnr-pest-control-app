...
import {
  isTransitionAllowed,
  BOOKING_TRANSITIONS,
} from "src/modules/bookings/enums";
...
describe("Booking state machine — isTransitionAllowed", () => {
  ...
  it("forbids skipping states (PENDING → COMPLETED)", () => {
    expect(
      isTransitionAllowed(BookingStatus.PENDING, BookingStatus.COMPLETED),
    ).toBe(false);
  });

  it("forbids moving backwards (IN_PROGRESS → PENDING)", () => {
    expect(
      isTransitionAllowed(BookingStatus.IN_PROGRESS, BookingStatus.PENDING),
    ).toBe(false);
  });
  ...
  it("allows NO_SHOW only from CONFIRMED and ARRIVED", () => {
    expect(
      isTransitionAllowed(BookingStatus.CONFIRMED, BookingStatus.NO_SHOW),
    ).toBe(true);
    expect(
      isTransitionAllowed(BookingStatus.ARRIVED, BookingStatus.NO_SHOW),
    ).toBe(true);
    expect(
      isTransitionAllowed(BookingStatus.EN_ROUTE, BookingStatus.NO_SHOW),
    ).toBe(false);
  });
});
