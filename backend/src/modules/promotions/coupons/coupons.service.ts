...
export class CouponsService {
  ...
  async create(dto: CreateCouponDto, actorId: string) {
    ...
    await this.audit(actorId, "coupon.created", coupon.id, {
      code: coupon.code,
    });
  ...
  async redeem(actor: AuthenticatedUser, dto: RedeemCouponDto) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const { coupon, discount } = await this.validation.assertRedeemable(
        dto.code,
        customerId,
        ctx,
      );
      ...
      return {
        ...
        final_amount: Math.max(0, ctx.base - discount),
      };
    });

    this.logger.log(
      `Coupon ${result.code} redeemed by customer ${customerId} (discount ${result.discount_amount})`,
    );
    return result;
  }
  ...
  async performance(id: string): Promise<CampaignPerformance> {
    ...
    return {
      ...
      unique_customers: distinct.length,
    ...
    if (!p) {
      if (actor.role === UserRole.ADMIN)
        throw new ForbiddenException({
          code: "FORBIDDEN",
          message: "Admins redeem on behalf via booking flows",
        });
      throw new BadRequestException({
  ...
  private audit(
    ...
    return this.prisma.auditLog.create({
      data: { actorId, action, entityType: "coupon", entityId, metadata },
    });
  }
