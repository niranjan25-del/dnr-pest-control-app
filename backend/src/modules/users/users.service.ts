...
export class UsersService {
  ...
  private isSuperAdmin(actor: AuthenticatedUser): boolean {
    return (
      actor.role === UserRole.ADMIN && actor.adminRole === AdminRole.SUPER_ADMIN
    );
  }
  ...
  async getMe(userId: string) {
    const user = await this.prisma.user.findFirst({
      ...
      include: {
        ...
        technicianProfile: { include: { serviceAreas: true } },
      },
    });
    if (!user)
    ...
    return {
      ...this.toPublic(base as UserRow),
      profile: customerProfile ?? technicianProfile ?? null,
    };
  }
  ...
  ): Promise<Paginated<ReturnType<UsersService["toPublic"]>>> {
  ...
  async setStatus(
    ...
    const user = await this.prisma.user.update({
      ...
      data: {
        status: dto.status,
        ...(dto.status === UserStatus.ACTIVE ? { deletedAt: null } : {}),
      },
      select: PUBLIC_SELECT,
    });
    await this.audit(actor.id, "user.status_changed", id, {
      status: dto.status,
      reason: dto.reason,
    });
  ...
  async setRole(id: string, dto: UpdateUserRoleDto, actor: AuthenticatedUser) {
    ...
    await this.audit(actor.id, "user.role_changed", id, {
      role: dto.role,
  ...
  async createAdmin(dto: CreateAdminDto, actor: AuthenticatedUser) {
    ...
    await this.audit(actor.id, "user.admin_created", user.id, {
      adminRole: dto.adminRole,
    });
    this.logger.warn(
      `Admin ${user.id} (${dto.adminRole}) created by ${actor.id}`,
    );
    return this.toPublic(user);
  }
  ...
  private async ensureExists(
    id: string,
  ): Promise<{ email: string; adminRole: AdminRole | null }> {
