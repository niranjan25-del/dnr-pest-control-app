...
export class WebhookController {
  ...
  async handleCashfree(
    ...
    if (!signature || !timestamp || !req.rawBody) {
      throw new BadRequestException({ code: 'INVALID_WEBHOOK', message: 'Missing signature, timestamp, or body' });
    }
    ...
    try {
      ...
      this.logger.warn(`Cashfree webhook signature verification failed: ${(err as Error).message}`);
      throw new BadRequestException({ code: 'INVALID_WEBHOOK', message: 'Signature verification failed' });
    }
    ...
    try {
      ...
      throw new BadRequestException({ code: 'INVALID_WEBHOOK', message: 'Invalid JSON body' });
    }
    ...
    try {
      ...
      this.logger.error(`Error handling Cashfree event ${event.type}: ${(err as Error).message}`);
    }
