...
import { LOCATION_NAMESPACE, LocationEvent, roomForBooking, roomForTechnician } from './enums';
import { UpdateLocationDto } from './dto';

@WebSocketGateway({ namespace: LOCATION_NAMESPACE, cors: { origin: true, credentials: true } })
export class LocationGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => LocationService)) private readonly location: LocationService,
  ) {}
  ...
  handleConnection(client: Socket) {
    try {
      ...
      client.emit(LocationEvent.ERROR, { code: 'UNAUTHORIZED', message: 'Authentication failed' });
  ...
  async unsubscribeBooking(@ConnectedSocket() client: Socket, @MessageBody() body: { bookingId: string }) {
  ...
  async onLocationUpdate(@ConnectedSocket() client: Socket, @MessageBody() body: UpdateLocationDto) {
    const user = this.user(client);
    return this.location.updateLocation(user, body); // service persists + broadcasts
  }

  // ---- broadcast helpers (called by LocationService) ----
  broadcastLocation(technicianId: string, bookingId: string | null, position: { latitude: number; longitude: number; accuracy?: number; recordedAt: Date }) {
    const payload = { technicianId, bookingId, ...position };
    this.server.to(roomForTechnician(technicianId)).emit(LocationEvent.LOCATION_UPDATE, payload);
    if (bookingId) this.server.to(roomForBooking(bookingId)).emit(LocationEvent.LOCATION_UPDATE, payload);
  }

  broadcastToBooking(bookingId: string, event: string, payload: Record<string, unknown>) {
    this.server.to(roomForBooking(bookingId)).emit(event, payload);
  }

  broadcastStatus(technicianId: string, event: string, payload: Record<string, unknown>) {
    this.server.to(roomForTechnician(technicianId)).emit(event, { technicianId, ...payload });
  }
  ...
  private authenticate(client: Socket): AuthenticatedUser {
    ...
    return { id: payload.sub, email: payload.email, role: payload.role as UserRole, adminRole: payload.adminRole ?? null, permissions: payload.permissions ?? [] };
  }
