...
export class LocationService {
  ...
  async startTracking(actor: AuthenticatedUser, dto: StartTrackingDto) {
    ...
    this.gateway.broadcastStatus(technicianId, LocationEvent.ONLINE, {
      status: session.status,
      bookingId: dto.bookingId ?? null,
    });
    await this.audit(actor.id, "tracking.started", technicianId, {
  ...
  async stopTracking(actor: AuthenticatedUser) {
    ...
    this.gateway.broadcastStatus(technicianId, LocationEvent.OFFLINE, {
      status: TechnicianTrackingStatus.OFFLINE,
    });
  ...
  async updateLocation(actor: AuthenticatedUser, dto: UpdateLocationDto) {
    ...
    await this.route.record(this.prisma, {
      technicianId,
      bookingId: dto.bookingId,
    ...
    const session = this.tracking.updatePosition(technicianId, {
      latitude: dto.latitude,
      longitude: dto.longitude,
      accuracy: dto.accuracy,
      recordedAt,
    ...
    if (dto.bookingId) {
      const arrived = await this.detectArrival(technicianId, dto.bookingId, {
        latitude: dto.latitude,
        longitude: dto.longitude,
      });
      if (arrived && !session.arrivedNotified) {
        ...
        this.gateway.broadcastToBooking(dto.bookingId, LocationEvent.ARRIVED, {
          technicianId,
        });
    ...
    this.gateway.broadcastLocation(technicianId, dto.bookingId ?? null, {
      latitude: dto.latitude,
      longitude: dto.longitude,
      accuracy: dto.accuracy,
      recordedAt,
  ...
  async checkIn(actor: AuthenticatedUser, dto: CheckInDto) {
    ...
    if (center) {
      const dist = this.geofence.distanceMeters(
        { latitude: dto.latitude, longitude: dto.longitude },
        center,
      );
      if (dist > CHECK_IN_RADIUS_M) {
        throw new BadRequestException({
          code: "CHECK_IN_OUTSIDE_RADIUS",
          message: `You are ${Math.round(dist)}m from the job site (limit ${CHECK_IN_RADIUS_M}m)`,
        });
      }
    } else {
      this.logger.warn(
        `Booking ${dto.bookingId} address has no coordinates — skipping check-in radius check`,
      );
    }
    ...
    this.tracking.setStatus(technicianId, TechnicianTrackingStatus.ARRIVED);
    this.gateway.broadcastToBooking(booking.id, LocationEvent.ARRIVED, {
      technicianId,
    });
    await this.notifyCustomer(
      booking.id,
      "Technician arrived",
      "Your technician has arrived for your service.",
    );
    this.logger.log(
      `Technician ${technicianId} checked in to booking ${booking.id}`,
    );
  ...
  async checkOut(actor: AuthenticatedUser, dto: CheckOutDto) {
    ...
    this.gateway.broadcastToBooking(booking.id, LocationEvent.COMPLETED, {
      technicianId,
    });
    await this.notifyCustomerCompleted(booking.id);
    this.logger.log(
      `Technician ${technicianId} checked out of booking ${booking.id}`,
    );
  ...
  async getCurrent(
    ...
    const position =
      session?.position ??
      (await this.route.lastKnown(technicianId).then(
        (r) =>
          r && {
            latitude: r.latitude,
  ...
  async getHistory(
    ...
    const selfTechId =
      actor.role === UserRole.TECHNICIAN
        ? await this.resolveTechnicianId(actor)
        : null;
    if (actor.role !== UserRole.ADMIN && selfTechId !== technicianId) {
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "Not authorized to view this location history",
      });
    }
  ...
  async getEta(
    actor: AuthenticatedUser,
    bookingId: string,
  ): Promise<EtaResult & { booking_id: string }> {
    ...
    const origin =
      session?.position ??
      (await this.route
        .lastKnown(technicianId)
        .then((r) => r && { latitude: r.latitude, longitude: r.longitude }));
    if (!origin)
      throw new BadRequestException({
        code: "TRACKING_NOT_ACTIVE",
        message: "Technician location is not available",
      });
    const dest = this.geofence.centerFromAddress(booking.address);
    if (!dest)
      throw new BadRequestException({
        code: "INVALID_COORDINATES",
        message: "Job address has no coordinates",
      });
  ...
  private async detectArrival(
    ...
    return center
      ? this.geofence.isWithin(point, center, GEOFENCE_ARRIVAL_RADIUS_M)
      : false;
  }
  ...
  private async assertCanView(actor: AuthenticatedUser, technicianId: string) {
    ...
    if (
      actor.role === UserRole.TECHNICIAN &&
      (await this.resolveTechnicianId(actor)) === technicianId
    )
      return;
    ...
    throw new ForbiddenException({
      code: "ACCESS_DENIED",
      message: "Not authorized to view this technician",
    });
  }
  ...
  private async assertCanViewBooking(
    ...
    if (actor.role === UserRole.CUSTOMER) {
      ...
      const own =
        cp &&
        (await this.prisma.booking.findFirst({
          where: { id: bookingId, customerId: cp.id },
          select: { id: true },
        }));
      if (own) return;
    }
    if (actor.role === UserRole.TECHNICIAN) {
      ...
      if (techId) {
        await this.assertAssigned(techId, bookingId);
        return;
      }
    }
    throw new ForbiddenException({
  ...
  private async requireTechnician(actor: AuthenticatedUser): Promise<string> {
    if (actor.role !== UserRole.TECHNICIAN && actor.role !== UserRole.ADMIN) {
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "Only technicians can share location",
      });
    }
  ...
  private async notifyCustomer(bookingId: string, title: string, body: string) {
    ...
    if (booking?.customer?.userId) {
      await this.dispatcher.dispatch(booking.customer.userId, {
  ...
  private async notifyCustomerCompleted(bookingId: string) {
    ...
    if (booking?.customer?.userId)
      await this.dispatcher.onBookingCompleted(
        bookingId,
  ...
  private audit(
    ...
    return this.prisma.auditLog.create({
      data: {
        actorId,
        action,
        entityType: "technician_location",
        entityId,
