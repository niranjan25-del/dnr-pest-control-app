...
export class EtaService {
  ...
  constructor(private readonly config: ConfigService) {
    this.apiKey =
      ...
      this.config.get<string>("maps.apiKey");
  }
  ...
  private async distanceMatrix(
    ...
    try {
      ...
      return {
        distance_m,
        duration_s,
        eta: new Date(Date.now() + duration_s * 1000),
        source: "google",
      };
    } catch (err) {
      this.logger.warn(
        `Distance Matrix failed, using estimate: ${(err as Error).message}`,
      );
      return null;
    }
  }
  ...
  private fallback(origin: GeoPoint, destination: GeoPoint): EtaResult {
    ...
    const duration_s = Math.round(
      (distance_m / 1000 / FALLBACK_SPEED_KMH) * 3600,
    );
    return {
      distance_m,
      duration_s,
