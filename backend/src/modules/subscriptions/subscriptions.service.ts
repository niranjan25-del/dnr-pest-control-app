...
export class SubscriptionsService {
  ...
  async create(actor: AuthenticatedUser, dto: CreateSubscriptionDto) {
    ...
    const addressId = await this.recurring.resolveAddressId(
      customerId,
      dto.addressId,
    );
    ...
    this.logger.log(
      `Subscription ${created} created for customer ${customerId} (plan ${plan.id})`,
    );
  ...
  async changePlan(id: string, actor: AuthenticatedUser, dto: ChangePlanDto) {
    ...
    const direction =
      Number(newPlan.price) >= Number(sub.plan.price)
        ? "upgraded"
        : "downgraded";
    await this.prisma.$transaction([
      this.prisma.subscription.update({
        ...
        data: {
          planId: newPlan.id,
          nextBillingDate: nextCycleDate(new Date(), newPlan.billingCycle),
        },
      }),
      this.prisma.auditLog.create({
        data: {
          actorId: actor.id,
          action: `subscription.${direction}`,
          entityType: "subscription",
          entityId: id,
    ...
    this.logger.log(
      `Subscription ${id} ${direction} to plan ${newPlan.id} by ${actor.id}`,
    );
    return this.findById(id, actor);
  }
      ...
      return {
        bookings: {
          some: {
            assignments: {
              some: {
                technicianId: p?.id ?? "00000000-0000-0000-0000-000000000000",
              },
            },
          },
        },
  ...
  private toResponse(s: SubRow) {
    return {
      ...
      plan: {
        ...
        billing_cycle: s.plan.billingCycle,
      },
