...
export class ServiceCategoriesService {
  ...
  async create(dto: CreateServiceCategoryDto) {
    const slug = await uniqueSlug(dto.name, (s) =>
      this.prisma.serviceCategory.findUnique({ where: { slug: s } }).then(Boolean),
    );
    const category = await this.prisma.serviceCategory.create({
      data: {
        name: dto.name, slug, description: dto.description, iconUrl: dto.iconUrl,
  ...
  async update(id: string, dto: UpdateServiceCategoryDto) {
    ...
    const category = await this.prisma.serviceCategory.update({
      where: { id },
      data: {
        name: dto.name, description: dto.description, iconUrl: dto.iconUrl,
  ...
  async findOne(id: string) {
    ...
    if (!category) throw new NotFoundException({ code: 'CATEGORY_NOT_FOUND', message: 'Service category not found' });
    return category;
  }
  ...
  async list(filter: CategoryFilterDto) {
    ...
    const [rows, total] = await this.prisma.$transaction([
      this.prisma.serviceCategory.findMany({
        where, orderBy: { [sort]: filter.order }, skip: filter.skip, take: filter.limit,
      }),
  ...
  private async ensure(id: string) {
    const exists = await this.prisma.serviceCategory.findFirst({ where: { id, deletedAt: null }, select: { id: true } });
    if (!exists) throw new NotFoundException({ code: 'CATEGORY_NOT_FOUND', message: 'Service category not found' });
  }
}
