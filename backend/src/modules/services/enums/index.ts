...
export const SERVICE_SORT_FIELDS = ['name', 'basePrice', 'estimatedDurationMin', 'createdAt'] as const;
...
export function safeSort<T extends readonly string[]>(
  ...
  return sort && (allowed as readonly string[]).includes(sort) ? (sort as T[number]) : fallback;
}
