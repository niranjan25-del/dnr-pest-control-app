...
export class WorkloadService {
  ...
  ): Prisma.TechnicianAssignmentWhereInput {
    return {
      technicianId,
  ...
  dailyWorkload(technicianId: string, ref = new Date()): Promise<number> {
    ...
    return this.prisma.technicianAssignment.count({
      where: this.countWhere(technicianId, start, end),
    });
  }
  ...
  async hasDailyCapacity(
    technicianId: string,
    ref = new Date(),
  ): Promise<boolean> {
    return (
      (await this.dailyWorkload(technicianId, ref)) < CAPACITY.MAX_DAILY_JOBS
    );
  }
      ...
      techs.map(async (t) => {
        ...
        return {
          ...
          dailyCapacityRemaining: Math.max(
            0,
            CAPACITY.MAX_DAILY_JOBS - dailyActive,
          ),
        };
      }),
    );
