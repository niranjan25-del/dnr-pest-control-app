...
export class TechnicianAssignmentService {
  ...
  async assign(actor: AuthenticatedUser, dto: AssignTechnicianDto) {
    ...
    if (!dto.overrideConstraints)
      await this.availability.assertAvailable(technicianId, window);
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      ...
      await this.audit(tx, actor.id, "assignment.created", a.id, {
        bookingId: booking.id,
        technicianId,
      });
      return a.id;
    ...
    this.logger.log(
      `Assignment ${created} created for booking ${booking.id} → tech ${technicianId} by ${actor.id}`,
    );
  ...
  async reassign(
    ...
    dto: ReassignTechnicianDto,
  ) {
    const current = await this.prisma.technicianAssignment.findUnique({
      where: { id },
    ...
    if (!ACTIVE_ASSIGNMENT_STATUSES.includes(current.status)) {
      throw new BadRequestException({
        ...
        message: `Cannot reassign a ${current.status} assignment`,
      });
    }
    ...
    const window = {
      start: current.booking.scheduledWindowStart,
      end: current.booking.scheduledWindowEnd,
    };
    if (!dto.overrideConstraints)
      await this.availability.assertAvailable(newTechId, window);
    ...
    this.logger.log(
      `Assignment ${id} reassigned → ${newId} (tech ${newTechId}) by ${actor.id}`,
    );
  ...
  async accept(id: string, actor: AuthenticatedUser) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.audit(tx, actor.id, "assignment.accepted", id, {
        bookingId: assignment.bookingId,
      });
    });
  ...
  async reject(id: string, actor: AuthenticatedUser, reason?: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.audit(tx, actor.id, "assignment.rejected", id, {
        bookingId: assignment.bookingId,
        reason,
      });
    });
  ...
  async cancel(id: string, actor: AuthenticatedUser) {
    const assignment = await this.prisma.technicianAssignment.findUnique({
      where: { id },
    });
    if (!assignment)
      throw new NotFoundException({
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.audit(tx, actor.id, "assignment.cancelled", id, {
        bookingId: assignment.bookingId,
      });
    });
    return { success: true };
  ...
  async availableTechnicians(bookingId?: string) {
    ...
    return (await this.workload.summaries()).filter(
      (w) => w.isAvailable && w.dailyCapacityRemaining > 0,
    );
  }
  ...
  async setOwnAvailability(
    ...
    await this.prisma.auditLog.create({
      data: {
        actorId: actor.id,
        action: "technician.availability_changed",
        entityType: "technician_profile",
        entityId: profile.id,
        metadata: { isAvailable: dto.isAvailable },
      },
    });
    this.logger.log(
      `Technician ${profile.id} availability → ${dto.isAvailable}`,
    );
  ...
  private assertTransition(from: AssignmentStatus, to: AssignmentStatus) {
    if (!isAssignmentTransitionAllowed(from, to)) {
      throw new BadRequestException({
        code: "INVALID_ASSIGNMENT_STATUS",
        message: `Cannot move assignment ${from} → ${to}`,
      });
    }
  }
  ...
  private async scope(
    actor: AuthenticatedUser,
  ): Promise<Prisma.TechnicianAssignmentWhereInput> {
    ...
    if (actor.role === UserRole.CUSTOMER) {
      ...
      return {
        booking: {
          customerId: p?.id ?? "00000000-0000-0000-0000-000000000000",
        },
      };
    }
  ...
  private audit(
    ...
    return tx.auditLog.create({
      data: {
        actorId,
        action,
        entityType: "technician_assignment",
        entityId,
        metadata,
      },
    });
  ...
  private toResponse(
    a: Prisma.TechnicianAssignmentGetPayload<{
    ...
    return {
      ...
      technician: {
        id: a.technicianId,
        name: a.technician.user.fullName,
        phone: a.technician.user.phone,
      },
      booking: {
        id: a.bookingId,
        status: a.booking.status,
        scheduled_start: a.booking.scheduledWindowStart,
        scheduled_end: a.booking.scheduledWindowEnd,
      },
