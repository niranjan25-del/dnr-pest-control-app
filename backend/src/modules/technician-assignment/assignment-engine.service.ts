...
export class AssignmentEngineService {
  ...
  private async loadBookingContext(bookingId: string) {
    const booking = await this.prisma.booking.findFirst({
      ...
      include: {
        address: { select: { postalCode: true, latitude: true, longitude: true } },
        service: { select: { slug: true, category: { select: { slug: true } }, pestCategory: { select: { slug: true } } } },
      },
    });
    if (!booking) throw new BadRequestException({ code: 'BOOKING_NOT_FOUND', message: 'Booking not found' });
    return booking;
  }

  /** Rank all eligible technicians for a booking (excludes hard-fails). */
  async rankCandidates(bookingId: string, excludeTechnicianId?: string): Promise<TechnicianCandidate[]> {
    const booking = await this.loadBookingContext(bookingId);
    const window: ResolvedWindow = { start: booking.scheduledWindowStart, end: booking.scheduledWindowEnd };
    ...
    for (const t of techs) {
      ...
      const techSkills = t.skills.map((s) => s.toLowerCase());
      ...
      if (loc && lat !== null && lng !== null) {
        distanceKm = this.haversineKm(lat, lng, Number(loc.latitude), Number(loc.longitude));
        // Linear falloff over 50 km.
        distanceScore = SCORE_WEIGHTS.DISTANCE * Math.max(0, 1 - Math.min(distanceKm, 50) / 50);
      }

      const score = Math.round(areaScore + skillScore + workloadScore + distanceScore);
      candidates.push({
        technicianId: t.id, name: t.user.fullName, score,
  ...
  async pickBest(bookingId: string, excludeTechnicianId?: string): Promise<TechnicianCandidate> {
    const ranked = await this.rankCandidates(bookingId, excludeTechnicianId);
    if (!ranked.length) {
      throw new BadRequestException({ code: 'TECHNICIAN_UNAVAILABLE', message: 'No eligible technician is available for this booking' });
    }
    this.logger.log(`Auto-assign for booking ${bookingId} → ${ranked[0].technicianId} (score ${ranked[0].score})`);
    return ranked[0];
  }
