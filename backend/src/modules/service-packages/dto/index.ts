...
import {
  ArrayMinSize, IsArray, IsBoolean, IsInt, IsNotEmpty, IsNumber, IsOptional, IsString, IsUUID,
...
export class CreatePackageDto {
  @IsString() @IsNotEmpty({ message: 'Package name is required' }) @MaxLength(150)
  name!: string;
  ...
  @IsArray() @ArrayMinSize(1, { message: 'A package must include at least one service' })
  @ValidateNested({ each: true }) @Type(() => IncludedServiceDto)
