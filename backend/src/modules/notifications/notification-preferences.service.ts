...
export class NotificationPreferencesService {
  ...
  updatePreferences(userId: string, dto: NotificationPreferenceDto): { preferences: ResolvedPreference[]; persisted: boolean } {
    ...
    const merged = this.getPreferences(userId).map((pref) => {
      ...
      return override ? { type: pref.type, push: override.push ?? pref.push, inApp: override.inApp ?? pref.inApp } : pref;
    });
    this.logger.warn(`Preference update for ${userId} not persisted (no preferences store in schema)`);
    return { preferences: merged, persisted: false };
  }

  // Default-on until a preferences store exists.
  isEnabled(_userId: string, _type: NotificationType, _channel: NotificationChannel): boolean {
    return true;
  }
}
