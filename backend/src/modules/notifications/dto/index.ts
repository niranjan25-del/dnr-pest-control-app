...
import {
  IsArray, IsBoolean, IsEnum, IsNotEmpty, IsObject, IsOptional, IsString, IsUUID, MaxLength,
  ValidateNested,
...
export class SendNotificationDto {
  ...
  @IsString() @IsNotEmpty({ message: 'title is required' }) @MaxLength(140)
  title!: string;

  @IsString() @IsNotEmpty({ message: 'body is required' }) @MaxLength(1000)
  body!: string;
...
export class RegisterDeviceDto {
  @IsString() @IsNotEmpty({ message: 'FCM token is required' })
  token!: string;
