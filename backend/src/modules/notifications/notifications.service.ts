...
export class NotificationsService {
  ...
  async list(actor: AuthenticatedUser, filter: NotificationFilterDto) {
    const where: Prisma.NotificationWhereInput = {
      userId: actor.id, ...(filter.type ? { type: filter.type } : {}),
    };
    const [rows, total] = await this.prisma.$transaction([
      this.prisma.notification.findMany({ where, orderBy: { createdAt: filter.order }, skip: filter.skip, take: filter.limit }),
  ...
  async unread(actor: AuthenticatedUser) {
    const where: Prisma.NotificationWhereInput = { userId: actor.id, readAt: null };
  ...
  async markRead(actor: AuthenticatedUser, id: string) {
    ...
    if (res.count === 0) {
      const exists = await this.prisma.notification.findFirst({ where: { id, userId: actor.id }, select: { id: true } });
  ...
  async remove(actor: AuthenticatedUser, id: string) {
    const res = await this.prisma.notification.deleteMany({ where: { id, userId: actor.id } });
    if (res.count === 0) throw new NotFoundException({ code: 'NOTIFICATION_NOT_FOUND', message: 'Notification not found' });
    return { success: true };
  }
  ...
  async removeDevice(actor: AuthenticatedUser, token: string) {
    const res = await this.prisma.deviceToken.deleteMany({ where: { token, userId: actor.id } });
    if (res.count === 0) throw new NotFoundException({ code: 'INVALID_DEVICE_TOKEN', message: 'Device token not found' });
    return { success: true };
  }
  ...
  async send(dto: SendNotificationDto) {
    const recipient = await this.prisma.user.findFirst({ where: { id: dto.userId, deletedAt: null }, select: { id: true } });
    if (!recipient) throw new NotFoundException({ code: 'USER_NOT_FOUND', message: 'Recipient not found' });
    return this.dispatcher.dispatch(dto.userId, { type: dto.type, title: dto.title, body: dto.body, data: dto.data });
  }

  broadcast(dto: BroadcastNotificationDto) {
    return this.dispatcher.broadcast(
      { type: dto.type ?? 'SYSTEM', title: dto.title, body: dto.body, data: dto.data },
      dto.role,
    );
  }

  private toResponse(n: { id: string; type: string; status: string; title: string; body: string; data: unknown; readAt: Date | null; createdAt: Date }) {
    return {
      id: n.id, type: n.type, status: n.status, title: n.title, body: n.body,
      data: n.data ?? null, read: n.readAt != null, read_at: n.readAt, created_at: n.createdAt,
    };
  }
}
