...
export class NotificationDispatcherService {
  ...
  async dispatch(
    ...
    const wantInApp = this.prefs.isEnabled(
      userId,
      payload.type,
      NotificationChannel.IN_APP,
    );
    const wantPush = this.prefs.isEnabled(
      userId,
      payload.type,
    ...
    if (wantPush) {
      ...
      if (tokens.length) {
        ...
        if (res.invalidTokens.length) {
          ...
          this.logger.log(
            `Pruned ${res.invalidTokens.length} invalid device token(s)`,
          );
        }
      }
    }
    ...
    this.logger.log(
      `Notification ${notification.id} → ${delivered ? "SENT" : "FAILED"} (push ${pushed})`,
    );
  ...
  async broadcast(
    ...
    const result = await this.dispatchMany(
      users.map((u) => u.id),
      payload,
    );
    ...
    this.logger.log(
      `Broadcast to ${result.created} user(s) [role=${role ?? "ALL"}]`,
    );
    return result;
  }
  ...
  async onBookingCreated(
    ...
    return this.dispatch(
      customerUserId,
      templates.bookingConfirmation({ bookingId, scheduledStart }),
    );
  }
  async onTechnicianAssigned(
    ...
    await this.dispatch(
      customerUserId,
      templates.technicianAssigned({ bookingId, technicianName }),
    );
    return this.dispatch(
      technicianUserId,
      templates.newAssignment({ bookingId, scheduledStart }),
    );
  }
  async onBookingEnRoute(bookingId: string, customerUserId: string) {
    return this.dispatch(
      customerUserId,
      templates.technicianEnRoute({ bookingId }),
    );
  }
  async onBookingCompleted(bookingId: string, customerUserId: string) {
    return this.dispatch(
      customerUserId,
      templates.bookingCompleted({ bookingId }),
    );
  }
  async onPaymentSuccess(
    ...
    return this.dispatch(
      customerUserId,
      templates.paymentSuccess({ amount, currency, invoiceNumber }),
    );
  }
  async onPaymentFailed(
    ...
    return this.dispatch(
      customerUserId,
      templates.paymentFailed({ amount, currency }),
    );
  }
  async onSubscriptionRenewal(
    customerUserId: string,
    planName: string,
    date: Date,
  ) {
    return this.dispatch(
      customerUserId,
      templates.subscriptionReminder({ planName, date }),
    );
  }
  async onNewChatMessage(
    ...
    return this.dispatch(
      recipientUserId,
      templates.newChatMessage({ conversationId, preview }),
    );
  }
}
