...
import {
  FCM_MAX_ATTEMPTS, FCM_MULTICAST_BATCH, FCM_RETRY_DELAY_MS, INVALID_TOKEN_ERRORS,
} from './enums';
...
export class FcmService {
  ...
  private messaging(): admin.messaging.Messaging | null {
    ...
    if (!admin.apps.length) {
      ...
      if (!projectId || !clientEmail || !privateKey) {
        this.enabled = false;
        this.logger.warn('Firebase not configured — push notifications disabled (in-app still works)');
        return null;
      }
  ...
  private toMessageData(data?: Record<string, unknown>): Record<string, string> {
  ...
  async sendToTokens(tokens: string[], payload: NotificationPayload): Promise<PushResult> {
    const result: PushResult = { attempted: tokens.length, succeeded: 0, failed: 0, invalidTokens: [] };
    ...
    for (let i = 0; i < tokens.length; i += FCM_MULTICAST_BATCH) {
      ...
      const res = await this.withRetry(() => messaging.sendEachForMulticast({
        tokens: batch,
        notification: { title: payload.title, body: payload.body },
        data: this.toMessageData(payload.data),
      }));
      if (!res) { result.failed += batch.length; continue; }
      res.responses.forEach((r, idx) => {
        if (r.success) result.succeeded++;
        else {
          result.failed++;
          if (r.error && INVALID_TOKEN_ERRORS.has(r.error.code)) result.invalidTokens.push(batch[idx]);
        }
      });
    }
    return result;
  ...
  async sendToTopic(topic: string, payload: NotificationPayload): Promise<boolean> {
    ...
    const res = await this.withRetry(() => messaging.send({
      topic, notification: { title: payload.title, body: payload.body }, data: this.toMessageData(payload.data),
    }));
    return Boolean(res);
  }
  ...
  private async withRetry<T>(fn: () => Promise<T>): Promise<T | null> {
    ...
    for (let attempt = 1; attempt <= FCM_MAX_ATTEMPTS; attempt++) {
      try {
        ...
        lastErr = err;
        if (attempt < FCM_MAX_ATTEMPTS) await new Promise((r) => setTimeout(r, FCM_RETRY_DELAY_MS * attempt));
      }
    }
    this.logger.error(`FCM send failed after ${FCM_MAX_ATTEMPTS} attempts: ${(lastErr as Error)?.message}`);
    return null;
  }
}
