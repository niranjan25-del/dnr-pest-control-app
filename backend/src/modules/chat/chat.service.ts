...
export class ChatService {
  ...
  // conversations
  createConversation(actor: AuthenticatedUser, dto: CreateConversationDto) {
    return this.conversations.createOrGet(actor, dto);
  }
  listConversations(actor: AuthenticatedUser, filter: ConversationFilterDto) {
    return this.conversations.list(actor, filter);
  }
  getConversation(actor: AuthenticatedUser, id: string) {
    return this.conversations.get(actor, id);
  }
  archiveConversation(actor: AuthenticatedUser, id: string) {
    return this.conversations.archive(actor, id);
  }
  ...
  getMessages(
    ...
    filter: MessageHistoryFilterDto,
  ) {
  ...
  deleteMessage(actor: AuthenticatedUser, messageId: string) {
    return this.messages.softDelete(actor, messageId);
  }
  globalUnread(userId: string) {
    return this.messages.globalUnread(userId);
  }
  ...
  async sendMessage(
    actor: AuthenticatedUser,
    dto: SendMessageDto,
  ): Promise<{ message: MessageView; recipientUserIds: string[] }> {
    const message = await this.messages.create(actor, dto);
    const recipientUserIds = (
      await this.conversations.participantUserIds(dto.conversationId)
    ...
    await Promise.all(
      recipientUserIds
        ...
        .map((id) =>
          this.dispatcher.onNewChatMessage(id, dto.conversationId, preview),
        ),
    );
