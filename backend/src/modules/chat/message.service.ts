...
export class MessageService {
  ...
  async create(
    ...
    const type =
      dto.type ?? (dto.mediaId ? MessageType.FILE : MessageType.TEXT);
    ...
    if (
      ...
      throw new BadRequestException({
        code: "VALIDATION_ERROR",
        message: "Attachment messages require a mediaId",
      });
    }
    if (dto.mediaId) {
    ...
    this.logger.log(
      `Message ${message.id} sent in ${dto.conversationId} by ${actor.id}`,
    );
  ...
  async softDelete(actor: AuthenticatedUser, messageId: string) {
    ...
    await this.prisma.chatMessage.update({
      where: { id: messageId },
      data: { content: DELETED_TOMBSTONE, mediaId: null },
    });
    return { success: true };
  }

  async globalUnread(userId: string): Promise<{
    total: number;
    per_conversation: { conversation_id: string; unread: number }[];
  }> {
    ...
    return {
      total: per.reduce((s, c) => s + c.unread, 0),
      per_conversation: per.filter((c) => c.unread > 0),
    };
  }
