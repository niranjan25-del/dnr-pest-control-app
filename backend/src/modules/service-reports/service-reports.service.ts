...
type ReportRow = Prisma.ServiceReportGetPayload<{
...
export class ServiceReportsService {
  ...
  async create(actor: AuthenticatedUser, dto: CreateServiceReportDto) {
    const technicianId = await this.requireTechnicianForBooking(
      actor,
      dto.bookingId,
    );
    const existing = await this.prisma.serviceReport.findFirst({
    ...
    if (existing)
      throw new ConflictException({
        code: "REPORT_EXISTS",
        message: "A report already exists for this booking",
      });
    ...
    await this.audit(actor.id, "report.created", report.id, {
      bookingId: dto.bookingId,
    });
    this.logger.log(
      `Service report ${report.id} created for booking ${dto.bookingId}`,
    );
    ...
    await this.prisma.$transaction(async (tx) => {
      if (dto.summary !== undefined || dto.recommendations !== undefined) {
        await tx.serviceReport.update({
          where: { id },
          data: { summary: dto.summary, recommendations: dto.recommendations },
        });
      }
      await this.replaceTextItems(
        tx,
        id,
        ReportItemLabel.FINDING,
        dto.findings,
      );
      await this.replaceTextItems(
        tx,
        id,
        ReportItemLabel.SERVICE,
        dto.services,
      );
      await this.replaceTextItems(
        tx,
        id,
        ReportItemLabel.SAFETY_NOTE,
        dto.safetyNotes,
      );
      await this.replaceTextItems(
        tx,
        id,
        ReportItemLabel.REGULATORY_NOTE,
        dto.regulatoryNotes,
      );

      if (dto.chemicals !== undefined) {
        await tx.serviceReportItem.deleteMany({
          where: { reportId: id, label: ReportItemLabel.CHEMICAL },
        });
      ...
      if (dto.beforePhotoMediaIds !== undefined)
        await this.replacePhotoItems(
          tx,
          id,
          ReportItemLabel.BEFORE_PHOTO,
          dto.beforePhotoMediaIds,
          actor,
        );
      if (dto.afterPhotoMediaIds !== undefined)
        await this.replacePhotoItems(
          tx,
          id,
          ReportItemLabel.AFTER_PHOTO,
          dto.afterPhotoMediaIds,
          actor,
        );
    });
  ...
  async submit(actor: AuthenticatedUser, id: string, dto: SubmitReportDto) {
    ...
    if (
      !report.summary?.trim() &&
      !report.items.some((i) => i.label === ReportItemLabel.SERVICE)
    ) {
      throw new BadRequestException({
        code: "VALIDATION_ERROR",
        message: "Add a summary or at least one service before submitting",
      });
    }
    await this.prisma.serviceReport.update({
      where: { id },
  ...
  async list(actor: AuthenticatedUser, filter: ReportFilterDto) {
    ...
    const views = await Promise.all(
      rows.map(async (r) =>
        this.toView(r, await this.signatures.hasSignature(r.id)),
      ),
    );
  ...
  private async replaceTextItems(
    ...
    if (clean.length)
      await tx.serviceReportItem.createMany({
        data: clean.map((value) => ({ reportId, label, value })),
      });
  }

  private async replacePhotoItems(
    tx: Prisma.TransactionClient,
    reportId: string,
    label: ReportItemLabel,
    mediaIds: string[],
    ...
    const media = await tx.mediaFile.findMany({
    ...
    for (const mid of mediaIds) {
      if (!found.has(mid))
        throw new BadRequestException({
          ...
          message: `Media ${mid} not found`,
        });
    }
    if (
      actor.role !== UserRole.ADMIN &&
      media.some((m) => m.uploaderId !== actor.id)
    ) {
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "You can only attach media you uploaded",
      });
    }
    await tx.serviceReportItem.createMany({
      data: mediaIds.map((mediaId) => ({ reportId, label, mediaId })),
    });
  }
  ...
  private assertTransition(from: ReportStatus, to: ReportStatus) {
    if (!isReportTransitionAllowed(from, to)) {
      throw new BadRequestException({
        ...
        message: `Cannot move report ${from} → ${to}`,
      });
    }
  }
  ...
  private async requireOwnedEditable(
    ...
    if (!EDITABLE_STATUSES.includes(report.status)) {
      throw new BadRequestException({
        code: "INVALID_STATUS_TRANSITION",
        message: `A ${report.status} report cannot be edited`,
      });
    }
    return report;
  }

  private async requireTechnicianForBooking(
    actor: AuthenticatedUser,
    bookingId: string,
  ): Promise<string> {
    const techId = await this.resolveTechnicianId(actor);
    if (!techId)
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "Only the assigned technician can create a report",
      });
    const assigned = await this.prisma.technicianAssignment.findFirst({
      where: { bookingId, technicianId: techId },
      select: { id: true },
    });
    if (!assigned && actor.role !== UserRole.ADMIN)
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
  ...
  private async assertCanView(actor: AuthenticatedUser, report: ReportRow) {
    ...
    if (
      ...
      report.technicianId === (await this.resolveTechnicianId(actor))
    )
      return;
  ...
  private async scope(
    actor: AuthenticatedUser,
  ): Promise<Prisma.ServiceReportWhereInput> {
    if (actor.role === UserRole.ADMIN) return {};
    ...
    if (actor.role === UserRole.CUSTOMER) {
      ...
      return {
        booking: {
          customerId: cp?.id ?? "00000000-0000-0000-0000-000000000000",
        },
      };
    }
  ...
  private toView(r: ReportRow, hasSignature: boolean): ServiceReportView {
    ...
    return {
      ...
      findings: by(ReportItemLabel.FINDING)
        .map((i) => i.value ?? "")
        .filter(Boolean),
      services: by(ReportItemLabel.SERVICE)
        .map((i) => i.value ?? "")
        .filter(Boolean),
      chemicals: by(ReportItemLabel.CHEMICAL).map((i) => ({
        chemicalName: i.chemicalName ?? "",
        quantity: i.quantity,
        area_notes: i.value,
      })),
      before_photo_media_ids: by(ReportItemLabel.BEFORE_PHOTO)
        .map((i) => i.mediaId)
        .filter((id): id is string => id != null),
      after_photo_media_ids: by(ReportItemLabel.AFTER_PHOTO)
        .map((i) => i.mediaId)
        .filter((id): id is string => id != null),
      safety_notes: by(ReportItemLabel.SAFETY_NOTE)
        .map((i) => i.value ?? "")
        .filter(Boolean),
      regulatory_notes: by(ReportItemLabel.REGULATORY_NOTE)
        .map((i) => i.value ?? "")
        .filter(Boolean),
