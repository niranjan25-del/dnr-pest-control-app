...
export class ServiceReportsService {
  ...
  private toView(r: ReportRow, hasSignature: boolean): ServiceReportView {
    ...
    return {
      ...
      before_photo_media_ids: by(ReportItemLabel.BEFORE_PHOTO).map((i) => i.mediaId).filter((id): id is string => id != null),
      after_photo_media_ids: by(ReportItemLabel.AFTER_PHOTO).map((i) => i.mediaId).filter((id): id is string => id != null),
