...
export class SignatureService {
  ...
  async store(reportId: string, uploaderId: string, imageBase64: string): Promise<{ media_id: string }> {
    ...
    try {
      ...
      throw new BadRequestException({ code: 'SIGNATURE_UPLOAD_FAILED', message: 'Failed to store the signature' });
    }
    const media = await this.prisma.$transaction(async (tx) => {
      // Retire any previous signature for this report.
      await tx.mediaFile.updateMany({ where: { ownerType: SIGNATURE_OWNER_TYPE, ownerId: reportId, deletedAt: null }, data: { deletedAt: new Date() } });
      return tx.mediaFile.create({
        data: {
          uploaderId, type: MediaType.IMAGE, storageKey: key, url: key, contentType: 'image/png',
          sizeBytes: buffer.byteLength, ownerType: SIGNATURE_OWNER_TYPE, ownerId: reportId,
        },
      });
    });
  ...
  async getMeta(reportId: string) {
    return this.prisma.mediaFile.findFirst({
      where: { ownerType: SIGNATURE_OWNER_TYPE, ownerId: reportId, deletedAt: null }, orderBy: { createdAt: 'desc' },
    });
  }
  ...
  async getBytes(reportId: string): Promise<Buffer | null> {
    ...
    if (!meta) return null;
    try {
      const url = await this.s3.presignGet(meta.storageKey, SIGNED_URL_TTL_SECONDS);
      const res = await fetch(url);
      return Buffer.from(await res.arrayBuffer());
    } catch (err) {
      this.logger.warn(`Could not fetch signature bytes for report ${reportId}: ${(err as Error).message}`);
      return null;
    }
  }
  ...
  private decodePng(input: string): Buffer {
    ...
    try {
      ...
      throw new BadRequestException({ code: 'SIGNATURE_UPLOAD_FAILED', message: 'Invalid signature encoding' });
    }
    // PNG magic bytes: 89 50 4E 47
    if (buffer.length < 8 || buffer[0] !== 0x89 || buffer[1] !== 0x50 || buffer[2] !== 0x4e || buffer[3] !== 0x47) {
      throw new BadRequestException({ code: 'SIGNATURE_UPLOAD_FAILED', message: 'Signature must be a PNG image' });
    }
    return buffer;
  }
}
