...
export class ReportGeneratorService {
  ...
  async generate(reportId: string): Promise<string> {
    ...
    const itemsBy = (label: string) =>
      report.items.filter((i) => i.label === label);
    const beforeKeys = itemsBy(ReportItemLabel.BEFORE_PHOTO)
      .map((i) => i.media?.storageKey)
      .filter(Boolean) as string[];
    const afterKeys = itemsBy(ReportItemLabel.AFTER_PHOTO)
      .map((i) => i.media?.storageKey)
      .filter(Boolean) as string[];
    ...
    const data: ReportPdfData = {
      ...
      customer: {
        ...
        address: report.booking.address
          ? `${report.booking.address.line1}, ${report.booking.address.city}, ${report.booking.address.state} ${report.booking.address.postalCode}`
          : undefined,
      },
      technician: {
        name: report.technician.user.fullName,
        license:
          (report.technician as { licenseNumber?: string }).licenseNumber ??
          null,
      },
      summary: report.summary,
      recommendations: report.recommendations,
      findings: itemsBy(ReportItemLabel.FINDING)
        .map((i) => i.value ?? "")
        .filter(Boolean),
      services: itemsBy(ReportItemLabel.SERVICE)
        .map((i) => i.value ?? "")
        .filter(Boolean),
      chemicals: itemsBy(ReportItemLabel.CHEMICAL).map((i) => ({
      ...
      safetyNotes: itemsBy(ReportItemLabel.SAFETY_NOTE)
        .map((i) => i.value ?? "")
        .filter(Boolean),
      regulatoryNotes: itemsBy(ReportItemLabel.REGULATORY_NOTE)
        .map((i) => i.value ?? "")
        .filter(Boolean),
    ...
    await this.prisma.mediaFile.upsert({
      ...
      create: {
        ...
        ownerType: REPORT_PDF_OWNER_TYPE,
        ownerId: reportId,
      },
    });
  ...
  async latestPdfKey(reportId: string): Promise<string | null> {
    const media = await this.prisma.mediaFile.findFirst({
      where: { ownerType: REPORT_PDF_OWNER_TYPE, ownerId: reportId },
