...
export function isReportTransitionAllowed(from: ReportStatus, to: ReportStatus): boolean {
  return REPORT_TRANSITIONS[from]?.includes(to) ?? false;
}

export const EDITABLE_STATUSES: ReportStatus[] = [ReportStatus.DRAFT, ReportStatus.REJECTED];
