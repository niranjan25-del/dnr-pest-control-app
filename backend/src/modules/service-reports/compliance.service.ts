...
export class ComplianceService {
  ...
  async chemicalUsage(params: {
    from?: string;
    to?: string;
    technicianId?: string;
  }): Promise<ChemicalUsageRow[]> {
    ...
    const items = await this.prisma.serviceReportItem.findMany({
      where,
      select: { chemicalName: true, quantity: true, reportId: true },
    });

    const map = new Map<
      string,
      { entries: number; reports: Set<string>; quantities: string[] }
    >();
