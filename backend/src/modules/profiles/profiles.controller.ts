...
export class ProfilesController {
  ...
  createCustomer(@CurrentUser('id') userId: string, @Body() dto: CreateCustomerProfileDto) {
    return this.profiles.createCustomerProfile(userId, dto);
  }

  @Patch('customer')
  @Roles(UserRole.CUSTOMER)
  updateCustomer(@CurrentUser('id') userId: string, @Body() dto: UpdateCustomerProfileDto) {
  ...
  createTechnician(@CurrentUser('id') userId: string, @Body() dto: CreateTechnicianProfileDto) {
    return this.profiles.createTechnicianProfile(userId, dto);
  }

  @Patch('technician')
  @Roles(UserRole.TECHNICIAN)
  updateTechnician(@CurrentUser('id') userId: string, @Body() dto: UpdateTechnicianProfileDto) {
