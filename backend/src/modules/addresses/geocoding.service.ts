...
export class GeocodingService {
  ...
  async geocode(addressLine: string): Promise<GeocodeResult | null> {
    ...
    let body: { status: string; results: Array<{
      geometry: { location: { lat: number; lng: number } };
      formatted_address: string; address_components: GoogleComponent[];
    }> };
    try {
      ...
      throw new BadRequestException({ code: 'GEOCODING_FAILED', message: 'Unable to validate address right now' });
    }

    if (body.status === 'ZERO_RESULTS' || !body.results?.length) {
      throw new BadRequestException({ code: 'INVALID_ADDRESS', message: 'Address could not be located' });
    }
    if (body.status !== 'OK') {
      this.logger.error(`Geocoding status ${body.status}`);
      throw new BadRequestException({ code: 'GEOCODING_FAILED', message: 'Address validation service error' });
    }

    const top = body.results[0];
    const get = (type: string) => top.address_components.find((c) => c.types.includes(type));
    return {
