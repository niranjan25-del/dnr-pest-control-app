...
export class AddressesService {
  ...
  async create(actor: AuthenticatedUser, dto: CreateAddressDto) {
    ...
    const address = await this.prisma.$transaction(async (tx) => {
      const existingCount = await tx.address.count({ where: { customerId, deletedAt: null } });
      ...
      return tx.address.create({
        data: {
          customerId, label: dto.label, line1: dto.line1, line2: dto.line2, city: dto.city,
          state: dto.state, postalCode: dto.postalCode, country: dto.country ?? 'IN',
          accessNotes: dto.accessNotes, latitude: coords?.latitude, longitude: coords?.longitude,
  ...
  async update(id: string, actor: AuthenticatedUser, dto: UpdateAddressDto) {
    ...
    const locationChanged = dto.line1 || dto.city || dto.state || dto.postalCode || dto.country;
    if (dto.latitude !== undefined && dto.longitude !== undefined) {
      coords = { latitude: dto.latitude, longitude: dto.longitude };
    } else if (locationChanged) {
      const geo = await this.ensureCoordinates({
        line1: dto.line1 ?? current.line1, city: dto.city ?? current.city,
        state: dto.state ?? current.state, postalCode: dto.postalCode ?? current.postalCode,
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      return tx.address.update({
        where: { id },
        data: {
          label: dto.label, line1: dto.line1, line2: dto.line2, city: dto.city, state: dto.state,
          postalCode: dto.postalCode, country: dto.country, accessNotes: dto.accessNotes,
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (current.isDefault) {
        const next = await tx.address.findFirst({
          where: { customerId, deletedAt: null }, orderBy: { createdAt: 'desc' }, select: { id: true },
        });
  ...
  async setDefault(id: string, actor: AuthenticatedUser) {
    ...
    await this.prisma.$transaction([
      this.prisma.address.updateMany({ where: { customerId, isDefault: true }, data: { isDefault: false } }),
  ...
  async checkEligibility(id: string, actor: AuthenticatedUser): Promise<EligibilityResult> {
    const customerId = await this.resolveCustomerId(actor);
  ...
  async assertBookableAddress(addressId: string, customerId: string) {
    const address = await this.prisma.address.findFirst({ where: { id: addressId, customerId, deletedAt: null } });
    if (!address) throw new BadRequestException({ code: 'ADDRESS_NOT_FOUND', message: 'Address not found for this customer' });
    const coverage = await this.serviceAreas.checkCoverage(address.postalCode);
    if (!coverage.covered) {
      throw new BadRequestException({ code: 'OUT_OF_SERVICE_AREA', message: 'We do not currently service this address' });
    }
    return address;
  }
  ...
  private async ensureCoordinates(parts: {
    ...
    const line = [parts.line1, parts.city, parts.state, parts.postalCode, parts.country ?? 'IN'].filter(Boolean).join(', ');
    const geo = await this.geocoding.geocode(line); // null if geocoding disabled
    return geo ? { latitude: geo.latitude, longitude: geo.longitude } : undefined;
  }

  private async resolveCustomerId(actor: AuthenticatedUser): Promise<string> {
    const profile = await this.prisma.customerProfile.findUnique({ where: { userId: actor.id }, select: { id: true } });
