...
export class ReportsService {
  ...
  private envelope(
    ...
    from: Date,
    to: Date,
  ): Pick<ReportEnvelope, "title" | "generated_at" | "range"> {
    return {
      title,
      ...
      sections: [
        {
          ...
          rows: [["Net Revenue", data.net_revenue]],
        },
        {
          heading: "Revenue Over Time",
          columns: ["Period", "Net"],
          rows: data.series.map((s) => [s.bucket, s.value]),
        },
        {
          heading: "Revenue by Service",
          columns: ["Service", "Net", "Payments"],
          rows: data.by_service.map((s) => [s.label, s.value, s.count ?? 0]),
        },
        {
          heading: "Revenue by Technician",
          columns: ["Technician", "Net"],
          rows: data.by_technician.map((s) => [s.label, s.value]),
        },
      ],
    };
  }
      ...
      sections: [
        {
          ...
          rows: [
            ...
            ["Reschedule Rate", data.reschedule_rate],
          ],
        },
        {
          heading: "By Status",
          columns: ["Status", "Count"],
          rows: data.by_status.map((s) => [s.label, s.value]),
        },
        {
          heading: "Trend",
          columns: ["Period", "Bookings"],
          rows: data.trends.map((t) => [t.bucket, t.value]),
        },
      ],
    };
  }
      ...
      sections: [
        {
          ...
          columns: ["Metric", "Value"],
          rows: [
            ["Total Customers", c.total_customers],
            ["New Customers", c.new_customers],
            ["Active Customers", c.active_customers],
            ["Repeat Customers", c.repeat_customers],
            ["Retention Rate", c.retention_rate],
            ["Avg Lifetime Value", c.avg_lifetime_value],
          ],
        },
      ],
    };
  ...
  async technicians(
    actor: import("../auth/interfaces/auth.interfaces").AuthenticatedUser,
    from: Date,
    to: Date,
  ): Promise<ReportEnvelope> {
      ...
      sections: [
        {
          ...
          rows: technicians.map((t) => [
            t.name,
            t.jobs_completed_total,
            t.jobs_completed_in_range,
            t.average_rating,
            t.revenue_generated,
            t.utilization_rate,
          ]),
      ...
      sections: [
        {
          ...
          rows: [
            ...
            ["Paused", s.paused_subscriptions],
            ["Cancelled (range)", s.cancelled_in_range],
            ["Churn Rate", s.churn_rate],
            ["Renewal Rate", s.renewal_rate],
