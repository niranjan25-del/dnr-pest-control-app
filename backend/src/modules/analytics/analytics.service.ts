...
export class AnalyticsService {
  ...
  range(from?: string, to?: string): { from: Date; to: Date } {
    ...
    const fromD = from
      ? new Date(from)
      : new Date(toD.getTime() - 30 * 86400_000);
  ...
  async revenueByService(from: Date, to: Date): Promise<LabelledValue[]> {
    ...
    return this.groupNet(
      payments,
      (p) => p.invoice?.booking?.service?.name ?? "Unattributed",
    );
  }
  ...
  async revenueByTechnician(from: Date, to: Date): Promise<LabelledValue[]> {
    ...
    return this.groupNet(
      payments,
      (p) =>
        p.invoice?.booking?.assignments?.[0]?.technician?.user?.fullName ??
        "Unassigned",
    );
  }

  private groupNet<
    T extends { amount: Prisma.Decimal; refundedAmount: Prisma.Decimal },
  >(rows: T[], keyFn: (r: T) => string): LabelledValue[] {
  ...
  async revenue(
    ...
    const [net, series, byService, byTechnician] = await Promise.all([
      this.revenueNet(from, to),
      this.revenueSeries(from, to, granularity),
      this.revenueByService(from, to),
      this.revenueByTechnician(from, to),
    ]);
    return {
      net_revenue: net,
      series,
  ...
  async bookings(
    ...
    const [total, byStatus, series, reschedules] = await Promise.all([
      ...
      this.prisma.bookingStatusHistory.count({
        where: {
          note: { contains: "reschedul", mode: "insensitive" },
          createdAt: { gte: from, lte: to },
        },
      }),
    ]);
    const count = (s: BookingStatus) =>
      byStatus.find((b) => b.status === s)?._count ?? 0;
    const completed = count(BookingStatus.COMPLETED);
    const cancelled =
      count(BookingStatus.CANCELLED) + count(BookingStatus.NO_SHOW);
    return {
      total,
  ...
  async customers(from: Date, to: Date) {
    const [total, newCustomers, activeCustomers, repeat] = await Promise.all([
      ...
      this.prisma.customerProfile.count({
        where: {
          deletedAt: null,
          bookings: { some: { createdAt: { gte: from, lte: to } } },
        },
      }),
      this.prisma.customerProfile.count({
        where: {
  ...
  async technicians(actor: AuthenticatedUser, from: Date, to: Date) {
    ...
    if (actor.role === UserRole.TECHNICIAN) {
      const self = await this.prisma.technicianProfile.findUnique({
      ...
      techs.map(async (t) => {
        const completedInRange = await this.prisma.booking.count({
          where: {
            ...
            assignments: { some: { technicianId: t.id } },
          },
        });
        return {
          technician_id: t.id,
          ...
          utilization_rate: this.estimateUtilization(
            completedInRange,
            from,
            to,
          ),
  ...
  private estimateUtilization(jobs: number, from: Date, to: Date): number {
    const days = Math.max(
      1,
      Math.ceil((to.getTime() - from.getTime()) / 86400_000),
    );
  ...
  async services(
    ...
    const names = await this.prisma.service.findMany({
      where: { id: { in: ids } },
    ...
    return {
      most_popular: popular
        .map((p) => ({
          label: nameMap.get(p.serviceId!) ?? "Unknown",
          value: p._count,
        }))
        .sort((a, b) => b.value - a.value),
  ...
  async subscriptions(from: Date, to: Date) {
    const [active, paused, cancelledInRange, plans] = await Promise.all([
      ...
      this.prisma.subscription.count({
        where: { deletedAt: null, status: SubscriptionStatus.PAUSED },
      }),
      this.prisma.subscription.count({
        where: {
          deletedAt: null,
  ...
  async reviews(
    ...
    return {
      ...
      distribution: dist
        .map((d) => ({ label: String(d.rating), value: d._count }))
        .sort((a, b) => Number(b.label) - Number(a.label)),
      satisfaction_rate: agg._count ? +(satisfied / agg._count).toFixed(4) : 0,
      trend: trend.map((t) => ({
        bucket: t.bucket.toISOString().slice(0, 10),
