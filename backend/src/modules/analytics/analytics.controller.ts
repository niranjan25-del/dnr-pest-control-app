...
import {
  BookingReportDto, CustomerReportDto, DashboardFilterDto, ExportReportDto, RevenueReportDto, TechnicianReportDto,
} from './dto';
...
export class AnalyticsController {
  ...
  bookings(@Query() q: BookingReportDto) {
    const { from, to } = this.analytics.range(q.from, q.to);
    return this.analytics.bookings(from, to, q.granularity ?? Granularity.DAILY);
  }
  ...
  technicians(@CurrentUser() actor: AuthenticatedUser, @Query() q: TechnicianReportDto) {
    const { from, to } = this.analytics.range(q.from, q.to);
  ...
  services(@Query() q: RevenueReportDto) {
    const { from, to } = this.analytics.range(q.from, q.to);
    return this.analytics.services(from, to, q.granularity ?? Granularity.MONTHLY);
  }
  ...
  @Roles(UserRole.ADMIN)
  reportRevenue(@CurrentUser() actor: AuthenticatedUser, @Body() q: RevenueReportDto) {
    const { from, to } = this.analytics.range(q.from, q.to);
  ...
  @Roles(UserRole.ADMIN)
  reportBookings(@CurrentUser() actor: AuthenticatedUser, @Body() q: BookingReportDto) {
    const { from, to } = this.analytics.range(q.from, q.to);
  ...
  @Roles(UserRole.ADMIN)
  reportCustomers(@CurrentUser() actor: AuthenticatedUser, @Body() q: CustomerReportDto) {
    const { from, to } = this.analytics.range(q.from, q.to);
  ...
  @Roles(UserRole.ADMIN)
  reportTechnicians(@CurrentUser() actor: AuthenticatedUser, @Body() q: TechnicianReportDto) {
    const { from, to } = this.analytics.range(q.from, q.to);
  ...
  @Post('export')
  @Roles(UserRole.ADMIN)
  async export(@CurrentUser() actor: AuthenticatedUser, @Body() dto: ExportReportDto) {
    const { from, to } = this.analytics.range(dto.from, dto.to);
    const report =
      dto.reportType === 'REVENUE' ? await this.reports.revenue(from, to, dto.granularity ?? Granularity.DAILY)
      : dto.reportType === 'BOOKINGS' ? await this.reports.bookings(from, to, dto.granularity ?? Granularity.DAILY)
      : dto.reportType === 'CUSTOMERS' ? await this.reports.customers(from, to)
      : dto.reportType === 'TECHNICIANS' ? await this.reports.technicians(actor, from, to)
      : await this.reports.subscriptions(from, to);
    const result = await this.exporter.export(report, dto.format);
    await this.audit(actor.id, 'analytics.export_generated', `${dto.reportType}:${dto.format}`);
    return result;
  }

  private audit(actorId: string, action: string, detail?: string) {
    return this.prisma.auditLog.create({ data: { actorId, action, entityType: 'analytics', entityId: detail ?? 'dashboard' } });
  }
}
