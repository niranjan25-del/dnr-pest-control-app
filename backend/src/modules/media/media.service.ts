...
export class MediaService {
  ...
  async upload(
    ...
    const { contentType, sizeBytes } = this.validator.validateBuffer(
      dto.category,
      file,
    );
    ...
    const fileUrl = this.s3.localMode
      ? this.s3.localUrl(key)
      : (this.cloudfront.publicUrl(key) ?? key);

    const media = await this.prisma.$transaction(async (tx) => {
      const created = await tx.mediaFile.create({
        data: {
          uploaderId: actor.id,
          type: mediaTypeFor(contentType),
          storageKey: key,
          url: fileUrl,
          contentType,
          sizeBytes,
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityType: "media",
          entityId: created.id,
          metadata: { category: dto.category, sizeBytes },
        },
      });
      return created;
    });
    this.logger.log(
      `Media ${media.id} uploaded by ${actor.id} (${dto.category}, ${sizeBytes}B)`,
    );
  ...
  async createSignedUpload(
    ...
    const policy = this.assertCategoryAllowed(actor, dto.category);
    const contentType = this.validator.validateDeclared(
      dto.category,
      dto.contentType,
      dto.sizeBytes,
    );
    ...
    const media = await this.prisma.mediaFile.create({
      data: {
        ...
        type: mediaTypeFor(contentType),
        storageKey: key,
        url: this.cloudfront.publicUrl(key) ?? key,
        contentType,
    ...
    await this.audit(actor.id, "media.signed_url_generated", media.id, {
      mode: "upload",
    });
    return {
      media_id: media.id,
  ...
  async replace(
    ...
    if (media.uploaderId !== actor.id && actor.role !== UserRole.ADMIN) {
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "You cannot replace this file",
      });
    }
  ...
  async restore(actor: AuthenticatedUser, id: string) {
    ...
    if (media.uploaderId !== actor.id && actor.role !== UserRole.ADMIN) {
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "You cannot restore this file",
      });
    }
    const updated = await this.prisma.mediaFile.update({
      where: { id },
    ...
    return this.toResponse(
      updated,
      this.categoryForOwnerType(updated.ownerType),
    );
  }
  ...
  async findById(actor: AuthenticatedUser, id: string): Promise<UploadedMedia> {
    const media = await this.requireMedia(id);
    if (!(await this.canAccess(actor, media)))
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
  ...
  async list(actor: AuthenticatedUser, filter: MediaFilterDto) {
    const where: Prisma.MediaFileWhereInput = {
      ...(filter.includeDeleted && actor.role === UserRole.ADMIN
        ? {}
        : { deletedAt: null }),
      ...(filter.category
        ? { ownerType: CATEGORY_CONFIG[filter.category].ownerType }
        : {}),
    ...
    return paginate(
      rows.map((m) =>
        this.toResponse(m, this.categoryForOwnerType(m.ownerType)),
      ),
      total,
      filter.page,
      filter.limit,
  ...
  async getAccessUrl(actor: AuthenticatedUser, id: string): Promise<AccessUrl> {
    ...
    const cf = this.cloudfront.signedUrl(
      media.storageKey,
      SIGNED_URL_TTL_SECONDS,
    );
    await this.audit(actor.id, "media.signed_url_generated", id, {
      mode: "download",
    });
    if (cf)
      return {
    ...
    const s3url = await this.s3.presignGet(
      media.storageKey,
      SIGNED_URL_TTL_SECONDS,
    );
  ...
  private assertCategoryAllowed(
    ...
    if (!policy.roles.includes(actor.role)) {
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "Your role cannot upload to this category",
      });
    }
    return policy;
  }
  ...
  private async assertOwnerAccess(
    actor: AuthenticatedUser,
    policy: { bookingLinked: boolean },
    ownerId?: string,
  ) {
    if (!policy.bookingLinked || !ownerId || actor.role === UserRole.ADMIN)
      return;
    const bookingIds = await this.actorBookingIds(actor);
    if (!bookingIds.includes(ownerId)) {
      throw new ForbiddenException({
        code: "ACCESS_DENIED",
        message: "You are not associated with this booking",
      });
    }
  }
  ...
  private async scope(
    actor: AuthenticatedUser,
  ): Promise<Prisma.MediaFileWhereInput> {
    if (actor.role === UserRole.ADMIN) return {};
    const bookingIds = await this.actorBookingIds(actor);
    return {
      OR: [
        { uploaderId: actor.id },
        ...(bookingIds.length ? [{ ownerId: { in: bookingIds } }] : []),
      ],
    };
  }
  ...
  private async actorBookingIds(actor: AuthenticatedUser): Promise<string[]> {
    ...
    if (actor.role === UserRole.TECHNICIAN) {
      ...
      if (!p) return [];
      const bookings = await this.prisma.booking.findMany({
        where: { assignments: { some: { technicianId: p.id } } },
        select: { id: true },
      });
      return bookings.map((b) => b.id);
    }
  ...
  private audit(
    ...
    return this.prisma.auditLog.create({
      data: { actorId, action, entityType: "media", entityId, metadata },
    });
  }
  ...
  private toResponse(m: MediaFile, category: MediaCategory): UploadedMedia {
    const fileName =
      m.storageKey
        .split("/")
        .pop()
        ?.replace(/^[0-9a-f-]{36}-/, "") ?? m.storageKey;
    return {
      id: m.id,
      category,
