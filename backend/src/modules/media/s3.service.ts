...
export class S3Service {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    if (this.localMode) {
      this.logger.warn(
        "AWS credentials not set — using local disk storage at ./uploads (dev only)",
      );
    }
  }
  ...
  async putObject(
    ...
    if (this.localMode) {
      ...
      this.logger.debug(
        `[local] stored ${key} (${body.length}B, ${contentType})`,
      );
      return;
    }
    try {
  ...
  async deleteObject(key: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `S3 deleteObject failed for ${key}: ${(err as Error).message}`,
      );
