...
export class FileValidatorService {
  ...
  validateBuffer(
    ...
    if (file.size > policy.maxBytes) {
      throw new BadRequestException({
        code: "FILE_TOO_LARGE",
        message: `File exceeds the ${Math.round(policy.maxBytes / (1024 * 1024))}MB limit for this category`,
      });
    }
    if (!policy.mimes.has(declared)) {
      throw new BadRequestException({
        code: "INVALID_FILE_TYPE",
        message: "File type is not allowed for this category",
      });
    }
  ...
  validateDeclared(
    ...
    if (sizeBytes && sizeBytes > policy.maxBytes) {
      throw new BadRequestException({
        code: "FILE_TOO_LARGE",
        message: "File exceeds the size limit for this category",
      });
    }
    return declared;
  }
