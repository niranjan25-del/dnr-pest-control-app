...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!user) throw new ForbiddenException({ code: 'FORBIDDEN_ROLE', message: 'Access denied' });

    if (requiredRoles?.length && !requiredRoles.includes(user.role)) {
      throw new ForbiddenException({ code: 'FORBIDDEN_ROLE', message: 'Your role cannot access this resource' });
    }

    if (requiredPermissions?.length) {
      const isSuperAdmin = user.role === UserRole.ADMIN && user.adminRole === AdminRole.SUPER_ADMIN;
      const hasAll = requiredPermissions.every((p) => user.permissions.includes(p));
      if (!isSuperAdmin && !hasAll) {
        throw new ForbiddenException({ code: 'INSUFFICIENT_PERMISSIONS', message: 'Missing required permission' });
      }
    }
