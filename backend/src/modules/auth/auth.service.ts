...
import {
  ChangePasswordDto, FirebaseLoginDto, LoginDto, RegisterDto, ResetPasswordDto, SelfRegisterRole,
} from './dto';
import { AuthResponse, AuthenticatedUser, PublicUser } from './interfaces/auth.interfaces';
...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<AuthResponse> {
    ...
    const user = await this.prisma.$transaction(async (tx) => {
      const created = await tx.user.create({
        data: { email: dto.email, phone: dto.phone, fullName: dto.fullName, passwordHash, role },
      });
  ...
  async login(dto: LoginDto): Promise<AuthResponse> {
    ...
    if (!user || !valid) {
      ...
      throw new UnauthorizedException({ code: AUTH_ERRORS.INVALID_CREDENTIALS, message: 'Invalid email or password' });
    }
    if (user.status !== UserStatus.ACTIVE) {
      throw new ForbiddenException({ code: AUTH_ERRORS.ACCOUNT_SUSPENDED, message: 'Account is not active' });
    }
  ...
  async firebaseLogin(identity: FirebaseIdentity, dto: FirebaseLoginDto): Promise<AuthResponse> {
    if (!identity.email) {
      throw new BadRequestException({ code: AUTH_ERRORS.INVALID_FIREBASE_TOKEN, message: 'Token missing email' });
    }
    let user = await this.prisma.user.findFirst({
      where: { OR: [{ firebaseUid: identity.uid }, { email: identity.email }], deletedAt: null },
    });
    ...
    if (!user) {
      ...
      user = await this.prisma.$transaction(async (tx) => {
        const created = await tx.user.create({
          data: {
            email: identity.email!, fullName: identity.name ?? identity.email!.split('@')[0],
            firebaseUid: identity.uid, role, emailVerified: identity.emailVerified,
          },
        });
        if (role === UserRole.CUSTOMER) await tx.customerProfile.create({ data: { userId: created.id } });
        else if (role === UserRole.TECHNICIAN) await tx.technicianProfile.create({ data: { userId: created.id } });
        return created;
      });
      this.logger.log(`Provisioned IdP user ${user.id} via ${identity.signInProvider ?? dto.provider ?? 'firebase'}`);
    } else if (!user.firebaseUid) {
      // Link an existing email account to this Firebase uid (deliberate, verified by token).
      user = await this.prisma.user.update({
        where: { id: user.id },
        data: { firebaseUid: identity.uid, emailVerified: user.emailVerified || identity.emailVerified },
      });
    }
  ...
  async forgotPassword(email: string): Promise<{ success: true; reset_token?: string }> {
    const user = await this.prisma.user.findFirst({ where: { email, deletedAt: null } });
  ...
  async resetPassword(dto: ResetPasswordDto): Promise<{ success: true }> {
    ...
    const user = decoded?.sub
      ? await this.prisma.user.findFirst({ where: { id: decoded.sub, deletedAt: null } })
      : null;
    if (!user) this.invalidReset();
    try {
      this.jwt.verify(dto.token, { secret: this.resetSecretFor(user!.passwordHash) });
    } catch {
      this.invalidReset();
    }
  ...
  async changePassword(userId: string, dto: ChangePasswordDto): Promise<{ success: true }> {
    ...
    if (!user?.passwordHash || !(await bcrypt.compare(dto.currentPassword, user.passwordHash))) {
      throw new UnauthorizedException({ code: AUTH_ERRORS.INVALID_CREDENTIALS, message: 'Current password is incorrect' });
    }
    const passwordHash = await bcrypt.hash(dto.newPassword, BCRYPT_ROUNDS);
  ...
  async getCurrentUser(userId: string): Promise<PublicUser> {
    ...
    if (!user) throw new UnauthorizedException({ code: 'UNAUTHORIZED', message: 'Account not found' });
    return this.toPublic(user);
  }

  // ---------- helpers ----------
  private invalidReset(): never {
    throw new BadRequestException({ code: AUTH_ERRORS.INVALID_RESET_TOKEN, message: 'Invalid or expired reset token' });
  }

  private toPublic(u: {
    id: string; email: string; fullName: string; phone: string | null; role: UserRole;
    adminRole: PublicUser['admin_role']; status: string; emailVerified: boolean;
  }): PublicUser {
    return {
      id: u.id, email: u.email, full_name: u.fullName, phone: u.phone, role: u.role,
      admin_role: u.adminRole, status: u.status, email_verified: u.emailVerified,
    };
  }
}
