...
import {
  IsEmail, IsEnum, IsNotEmpty, IsOptional, IsString, Matches, MaxLength, MinLength,
...
export class RegisterDto {
  ...
  @IsString() @IsNotEmpty({ message: 'Full name is required' })
  @MaxLength(120)
  fullName!: string;
...
export class RefreshTokenDto {
  @IsString() @IsNotEmpty({ message: 'refresh_token is required' })
...
export class ResetPasswordDto {
  @IsString() @IsNotEmpty({ message: 'Reset token is required' })
  token!: string;
...
export class ChangePasswordDto {
  @IsString() @IsNotEmpty({ message: 'Current password is required' })
...
export class FirebaseLoginDto {
  @IsString() @IsNotEmpty({ message: 'Firebase ID token is required' })
  idToken!: string;
