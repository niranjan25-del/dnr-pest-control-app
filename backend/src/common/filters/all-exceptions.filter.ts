...
export class AllExceptionsFilter implements ExceptionFilter {
  private readonly logger = new Logger('Exception');
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof body === 'string') {
        ...
        message = (Array.isArray(b.message) ? 'Validation failed' : (b.message as string)) ?? message;
    ...
    if (status >= 500) {
      this.logger.error(`[${requestId}] ${req.method} ${req.url} → ${status} ${code}`, (exception as Error)?.stack);
    } else {
      this.logger.warn(`[${requestId}] ${req.method} ${req.url} → ${status} ${code}`);
    }

    const payload: ErrorResponse = { error: { code, message, details, request_id: requestId } };
  ...
  private mapPrisma(e: Prisma.PrismaClientKnownRequestError): { status: number; code: string; message: string } {
    switch (e.code) {
      ...
      case 'P2003':
        return { status: HttpStatus.BAD_REQUEST, code: 'FK_CONSTRAINT', message: 'Related resource constraint failed' };
      default:
        return { status: HttpStatus.INTERNAL_SERVER_ERROR, code: 'DB_ERROR', message: 'Database error' };
    }
  }
}
