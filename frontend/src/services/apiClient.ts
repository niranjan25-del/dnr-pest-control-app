...
function toApiError(error: AxiosError<ApiErrorEnvelope>): ApiError {
  ...
  // Custom envelope: { error: { code, message, details, request_id } }
  ...
  // NestJS native formats: { code?, message: string } or { message: string[] } (validation)
  const data = error.response?.data as Record<string, unknown> | undefined;
  if (data?.message) {
    const raw = data.message;
    const msg = Array.isArray(raw) ? (raw[0] as string) : typeof raw === 'string' ? raw : null;
    if (msg) {
      return new ApiError({ code: (data.code as string | undefined) ?? 'ERROR', message: msg, status });
    }
  }
